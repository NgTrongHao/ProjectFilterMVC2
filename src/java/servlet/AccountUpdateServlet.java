...
public class AccountUpdateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = ApplicationConstants.UpdateAccountFeatures.ERROR_PAGE;
        ...
        try {
            ...
            //3. process result
            if (result) {
                //refresh page --> call previous function (search) --> using url re-writing
                url = "searchAction"
                        + "?txtSearchValue=" + lastSearchValue;
            }
        } catch (SQLException ex) {
            log("AccountUpdateServlet_SQL: " + ex.getMessage());
        } catch (NamingException ex) {
            log("AccountUpdateServlet_Naming: " + ex.getMessage());
        } finally {
